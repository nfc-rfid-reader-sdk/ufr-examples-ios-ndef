...
class BluetoothViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        txtMacAddress.delegate = self
        
        self.view.backgroundColor = UIColor(patternImage: UIImage(named: "background")!)
    }
    
    @IBAction func writeBluetooth(_ sender: Any) {
        ...
        {
            alert.message = "Invalid Mac address input!"
        ...
        if (status == UFR_OK)
        {
            alert = UIAlertController(title: "Write Bluetooth NDEF was successful", message: "Status: \(strStatus)", preferredStyle: .alert)
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
                alert.dismiss(animated: true, completion: nil)
            }
        } else
        {
            alert = UIAlertController(title: "Write Bluetooth NDEF failed!", message: "Status: \(strStatus)", preferredStyle: .alert)
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
                alert.dismiss(animated: true, completion: nil)
            }
        }
